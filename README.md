...
- 正式フォント：日本語 `Noto Sans JP`／英数字 `Inter`
- 所在地：愛知県青澄市つむぎ町2-8-10（完全架空）
- 営業時間：平日 9:00〜18:00
- 定休日：土・日・祝
...
- 無料相談フォームは入力可能なUIと入力チェックを実装する
- JavaScriptによるダミー送信とし、外部サービス、サーバー、実在するメールアドレス等へデータを送信しない
- 送信後は完了メッセージを表示し、実際の予約・データ送信は行われていないことを明示する
- プライバシーポリシーは実運用ページを作成せず、フォーム付近またはFooterのダミーリンクとして扱う
- Footer内または直下に、ポートフォリオ用自主制作の架空サイトであることを小さく表示する
